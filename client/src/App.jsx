...
root.render(<App />);`,
  ...
  const skipAutoSaveRef = useRef(false); // ✅ Prevent auto-save conflict after MongoDB save
  ...
  useEffect(() => {
    if (skipAutoSaveRef.current) return; // skip if MongoDB just saved
  ...
  const handleSave = async () => {
    try {
      skipAutoSaveRef.current = true; // temporarily skip local auto-save
      ...
      // Allow auto-save again after 1 second
  ...
  return (
    ...
    >
      <div className={`app-container ${theme}`}>
        {/* Auto-save animation */}
        {isSaving && <div className="autosave-bubble">💾 Saving locally...</div>}
